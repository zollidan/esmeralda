...
from app.parser.soccerway.request_functions import *
...
def get_macthes_from_leage(leage_info):
    ...
    link = 'https://ru.soccerway.com/a/block_livescores?callback_params={}&block_id=comp&action=loadcomp&params={"d":"' + date_str + '","c":' + leage_id + '}'
...
def get_matches(response, date_str):
    ...
    try:
        ...
        for i, leage in enumerate(list(soup.find_all('div', class_='livescores-comp'))):
...
def get_change_page_link(home_team_id, away_team_id, page):
    ...
    link = "https://ru.soccerway.com/a/block_h2h_matches?" + (urllib.parse.urlencode(get_params))
    return link
...
def get_filtered_matches_link(team_id, filter, page):
    ...
    link = "https://ru.soccerway.com/a/block_team_matches?" + (urllib.parse.urlencode(get_params))
    return link
